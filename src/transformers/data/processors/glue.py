...
def _glue_convert_examples_to_features(
    ...
    def label_from_example(example: InputExample) -> Union[int, float, list,None]:
        ...
        if output_mode=='multilabel_classification':
            print([float(i) for i in example.label])
...
class Nlpcct5Level1Processor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        with open(os.path.join(data_dir, "train_set.json"),'r') as f:
            lst = eval(f.read())
            return self._create_examples(lst, "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        with open(os.path.join(data_dir, "val_set.json"),'r') as f:

            return self._create_examples(eval(f.read()), "dev")
